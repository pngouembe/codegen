...
class CTypes(LanguageSpecificType):
    @classmethod
    def from_internal(cls, internal: InternalType):
        internal.namespace_sep = c_NAMESPACE_SEP
        ...
        for m in [mod.value for mod in CTypeModifier]:
            if m in tmp_str:
        ...
        for s in str_list:
            ...
            try:
                ...
                if c_NAMESPACE_SEP in s:
                    s = s.rsplit(c_NAMESPACE_SEP, maxsplit=1)[-1]
                    try:
                        include_file = INCLUDE_FILE_MATCHER_DICT[s]
                        if include_file != "builtin":
                            INCLUDE_FILES_SET.add(INCLUDE_FILE_MATCHER_DICT[s])
                    ...
                    # TODO: Create a forward definition
                    INCLUDE_WARNINGS_SET.add(
                        f"Warning {s} is an unknown type")
        return cls(name=internal.name, namespace=internal.namespace, namespace_sep=c_NAMESPACE_SEP)
