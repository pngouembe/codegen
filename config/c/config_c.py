...
class CConfigObj(ConfigObj):
    ...
    def __post_init__(self) -> None:
        ...
        if self.custom_include_file_dict_path:
            self.custom_include_file_dict_path = _resolve_path(
                self.custom_include_file_dict_path, self.root_path)
            try:
                with open(self.custom_include_file_dict_path, 'r') as f:
                    self.custom_include_file_dict = yaml.safe_load(f)
            except FileNotFoundError:
                log.warn(
                    f"{self.custom_include_file_dict_path} file not found: only std-library includes will be used.")
